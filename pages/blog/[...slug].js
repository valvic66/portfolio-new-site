...
const getFilteredPosts = (dateFilter, dataToFilter) => {
  ...
  return dataToFilter.filter((posts) => {
    const date = new Date(posts.data.date);

    return date.getFullYear() === year && date.getMonth() === month - 1;
  });
};

function FiltedPostsPage() {
  ...
  const filteredPosts = getFilteredPosts(filterData, posts);
  ...
  if (false) {
    return (
      <div className="flex justify-center items-center h-screen">
        <svg
          aria-hidden="true"
          className="mr-2 w-10 h-10 text-gray-100 animate-spin dark:text-gray-400 fill-blue-600"
          viewBox="0 0 100 101"
          fill="none"
